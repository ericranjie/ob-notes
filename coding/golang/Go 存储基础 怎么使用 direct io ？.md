...
1. direct io 需要 io 大小和偏移扇区对齐，且还要满足内存 buffer 地址的对齐，这个怎么做到？
...
1. 减少内核 page cache 的内存使用，业务层自己控制内存，更加灵活；
...
1. 然后在 \[ p1, p1+512 \] 这个地址范围找，一定能找到 512 对齐的地址（这个能理解吗？），假设这个地址是 p2 ;

1. 返回 p2 这个地址给用户使用，用户能正常使用 \[ p2, p2 + 4096 \] 这个范围的内存块而不越界；
...
1. O_DIRECT 不在 os 库，而在于操作系统相关的 syscall 库；

1. Go 中无法直接使用 make 来分配对齐内存，一般的做法是分配一块大一点的内存，然后在里面找到对齐的地址即可；
...
\](http://mp.weixin.qq.com/s?\_\_biz=Mzg3NTU3OTgxOA==&mid=2247493867&idx=1&sn=9fef8e55b8220976d6362658d5188618&chksm=cf3df82ef84a7138f21b6fea9e719b204b0f7adaab5632cc8aff8b6966c985300d4af2a71bcd&scene=21#wechat_redirect)
...
\](http://mp.weixin.qq.com/s?\_\_biz=Mzg3NTU3OTgxOA==&mid=2247490768&idx=1&sn=ff12c3823968c75b8ee3d7027254a614&chksm=cf3e0c15f8498503df5575eaceca3c87da8807e044f633f8ba79b62bfd6b296a82ec7354c907&scene=21#wechat_redirect)

\[

Go 存储基础 — 文件 IO 的姿势

\](http://mp.weixin.qq.com/s?\_\_biz=Mzg3NTU3OTgxOA==&mid=2247489085&idx=1&sn=1eb51ea0a00a00a7a62f232221bafaf8&chksm=cf3e06f8f8498fee063670f21bd0a756ef821e359709ed62720b48927ad18eeb0757399036a8&scene=21#wechat_redirect)
