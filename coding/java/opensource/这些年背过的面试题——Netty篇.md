...
_2024年03月29日 08:31_ _浙江_

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/Z6bicxIx5naITrxXv5hOIZiaErdh9JqMDtibgKNng0xkfncj1fkRG7udr13sojicK1ibXP86gibeGibURbSsg3Sic8hNQA/640?wx_fmt=jpeg&from=appmsg&wxfrom=13&tp=wxpic)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/Z6bicxIx5naKlSyXlfVRJVKMz6APuG8o2uPV9clQbw2JicRqjtIqGia10icUI7DC3VaxtWsxga6iaD7Ztb6xBzBvJjA/640?wx_fmt=png&from=appmsg&wxfrom=13&tp=wxpic)
...
1. 当进行网络 I/O 操作、文件读写时，堆内内存都需要转换为堆外内存，然后再与底层设备进行交互，所以直接使用堆外内存可以减少一次内存拷贝。

1. 堆外内存可以方便实现进程之间、JVM 多实例之间的数据共享。
...
1. 请求的数据从内核态缓冲区拷贝到用户态缓冲区，然后返回给用户进程。**第二次数据\*\*\*\*拷贝**的过程同时，会导致上下文从内核态再次切换到用户态。

1. 用户进程调用 send() 方法期望将数据发送到网络中，用户态会再次切换到内核态，**第三次数据拷贝**请求的数据从用户态缓冲区被拷贝到 Socket 缓冲区。

1. 最终 send() 系统调用结束返回给用户进程，发生了第四次上下文切换。**第四次拷贝会异步执行**，从 Socket 缓冲区拷贝到协议引擎中。
...
上一篇这些年背过的面试题——JVM篇下一篇这些年背过的面试题——LeetCode
