...
Linux爱好者 _2021年10月07日 11:50_

The following article is from 奇伢云存储 Author 奇伢

![[Pasted image 20241019131130.png]]
...
![[Pasted image 20241019131936.png]]

不同层次做不同的事情，不断的封装，不断的站在巨人的肩膀上，你将能做的更多。

![[Pasted image 20241019132017.png]]
...
1. 服务端用 `bind` 绑定监听地址，用 `listen` 把套接字转化为监听套接字，用 `accept` 捞取一个客户端来的连接；

1. 客户端用 `connect` 进行建连，用 `write/read` 进行网络 IO；
...
1. `bind( )`  绑定一个端口（和客户端约定好的知名端口号）；

1. `listen( )` 讲套接字转化成监听套接字；

1. `accept( )` 等待客户端的建连请求；

1. 建连之后 read/write 处理数据即可（一般和监听线程并发）；

**客户端：**

1. `socket( )` 创建出 socketfd；

1. `connect( )` 向指定机器、端口发起建连请求；

1. 建连之后，read/write 处理数据；
...
1. 第二件事：调用 `sock_map_fd` 函数创建好 `struct file` 这个结构体，并与第一步创建出的 `struct socket` 关联起来；
...
1. `socket( )` 返回的是非负整数的 fd，与 `struct file` 对应，而 `struct file` 则与具体的 `struct socket` 关联，从而实现**一切皆文件**的封装的一部分（另一部分 inode 的创建处理在 sock_alloc 的函数里体现）；
...
1. 创建了 `struct sock` 对象，并且把 `struct sock` 初始化，并和 `struct socket` 进行关联；
...
1. 然后把 address 和 socket 绑定对应起来（调用 `sock->ops->bind` 函数）；
...
1. 调用 `sock->ops->listen` 函数（对应 `inet_listen` ）；
...
1. 调用 `inet_csk_listen_start` ；

`inet_csk_listen_start` 做啥了？

1. 初始化请求队列 `icsk->icsk_accept_queue` ；

1. 套接字状态设置成 `TCP_LISTEN`；

1. 获取到之前 bind 的端口，如果没有设置，那么就会用个临时的端口；

1. 把监听套接字加入到全局 hash 表中；
...
1. sk buffer 有可写的空间，触发可写事件；

1. 句柄杯 close，连接关闭的时候，也是可读的；
...
1. 把阻塞在 `epoll_wait` 的线程（Linux 进程和线程本质无区别）投递到就绪队列中，等待内核调度（也就是所谓的唤醒，实现机制很简单，就是 `epoll_wait` 阻塞切走之前，会创建出一个 wait 对象，挂到 epoll 池上，后续唤醒就能以此为依据）；
...
1. socket fd 能够和文件 IO 一样，使用 write/read 等系统调用，就得益于 vfs 帮你做的转接。那 `socket()` 函数调用是不是就和 open 文件 fd 的效果是一样的呀？是的，都是构建并关联各种内核结构体；

1. epoll 池能管理 socketfd，因为 socket fd 实现 poll 接口；

1. epoll_ctl 注册 socket fd 的时候，挂了个 wait 对象在 socket 的 sk_wq 里，所以数据就绪的时候，socket 才能通知到 epoll；

1. epoll_wait 切走的时候挂了个 wait 对象在 epoll 上，所以 epoll 就绪的时候，才能有机会唤醒阻塞的线程；

1. **套接字由 `socket()` 创建出来**，客户端和服务端都是，`listen()` 调用可以把套接字转化成监听套接字；

1. 监听套接字一般只监听可读事件，关注连接的建立，普通套接字走数据流，关注数据的读写事件；
