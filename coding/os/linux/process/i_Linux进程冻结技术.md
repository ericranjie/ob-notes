...
1. if (likely(!freezing(current))) //检查系统是否处于freezing状态

1. ```
    return false;
   ```

1. return \_\_refrigerator(false); //主动进入冻结

1. }

1. static inline bool freezing(struct task_struct \*p)
...
1. bool freezing_slow_path(struct task_struct \*p)
...
bool \_\_refrigerator(bool check_kthr_stop)
...
## 有两个问题请帮忙解答，先行谢过<br /> 1）因为只有指明了freezable的内核线程才会进入freeze, 意思是在suspend后，其实系统中默认的内核线程原来处于running状态的依然在running状态吗？<br /> 2）那些本来就已经在uninterruptable 状态的用户线程是否也要响应freeze请求进入freeze状态呢？如果是的话，这些状态的线程没有机会来响应这个请求吧？

## static int try_to_freeze_tasks(bool user_only)<br /> 48                 do_each_thread(g, p) {   //这里应该遍历所有状态的线程，不管是否已经sleep了<br /> 49                         if (p == current || !freeze_task(p))<br /> 50                                 continue;
...
## @nowar：可惜发布者可能太忙了，好像很久没有更新了。<br /> 因为最近也在看这个部分，就借用这个平台一起讨论下吧。
