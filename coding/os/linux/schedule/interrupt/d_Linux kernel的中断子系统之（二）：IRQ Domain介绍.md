...
> {\
> return \_\_irq_domain_add(of_node, size, size, 0, ops, host_data);\
> }
...
> void \*host_data)\
> {\
> return \_\_irq_domain_add(of_node, 0, ~0, 0, ops, host_data);\
> }
...
> void \*host_data)\
> {\
> return \_\_irq_domain_add(of_node, 0, max_irq, max_irq, ops, host_data);\
> }

这类接口的逻辑很简单，根据自己的映射类型，初始化struct irq_domain中的各个成员，调用\_\_irq_domain_add将该irq domain挂入irq_domain_list的全局列表。
...
>
> domain = \_\_irq_domain_add(of_node, first_hwirq + size,－－－－注册irq domain\
...
> void \_\_init gic_cascade_irq(unsigned int gic_nr, unsigned int irq)\
> {\
...
## "<br /> （1）首先找到root interrupt controller对应的irq domain。<br /> （2）根据HW 寄存器信息和irq domain信息获取HW interrupt ID<br /> "
...
但是每个bank设置了\_\_irq_do_set_handler，　同时pin ctrl也设置了对应的\_\_irq_do_set_handler，同时gic controller也有对应的irq_handler.\
...
\[    2.214551\]  \[<c19ecb88>\] ? \_\_irq_alloc_descs+0xe8/0x1b0\
...
\[    2.214910\]  \[<c16ade09>\] \_\_driver_attach+0x79/0x80\
\[    2.214933\]  \[<c16add90>\] ? \_\_device_attach+0x40/0x40\
\[    2.214955\]  \[<c16abdaf>\] bus_for_each_dev+0x4f/0x80\
\[    2.214979\]  \[<c16ad51e>\] driver_attach+0x1e/0x20\
\[    2.215001\]  \[<c16add90>\] ? \_\_device_attach+0x40/0x40\
...
\[    2.215168\]  \[<c161ab9a>\] \_\_pci_register_driver+0x4a/0x50\
