...
|   |
|---|
|static __always_inline void queued_spin_lock(struct qspinlock *lock)<br><br>{<br><br> u32 val = 0;<br><br> if (likely(atomic_try_cmpxchg_acquire(&lock->val, &val, _Q_LOCKED_VAL)))<br><br>   return;<br><br> queued_spin_lock_slowpath(lock, val);<br><br>}|
