...
（3）可抢占RCU（RCU-preempt），也称为实时RCU，在内核版本2.6.26中引入。可抢占RCU允许进程在读端临界区被其他进程抢占。编译内核时需要开启配置宏CONFIG\_ PREEMPT_RCU。

RCU根据数据结构可以分为以下两种。

（1）树型RCU（tree RCU）：也称为基于树的分层RCU（Tree-based hierarchical RCU），为拥有几百个或几千个处理器的大型系统设计。配置宏是CONFIG_TREE_RCU。
（2）微型RCU（tiny RCU）：为不需要实时响应的单处理器系统设计。配置宏是CONFIG\_ TINY_RCU。
