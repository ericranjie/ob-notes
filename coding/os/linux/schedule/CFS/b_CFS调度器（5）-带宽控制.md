...
> 1. 设定的定时器周期时间。
> 1. 限额时间。
> 1. 剩余可运行时间，在每次定时器回调函数中更新值为quota。
> 1. 上面一直提到的高精度定时器。
> 1. 所有被throttle的cfs_rq挂入此链表，在定时器的回调函数中便利链表执行unthrottle cfs_rq操作。
...
> 1. cfs_rq所属的task_group。
> 1. 该就绪队列是否已经开启带宽限制，默认带宽限制是关闭的，如果带宽限制使能，runtime_enabled的值为1。
> 1. cfs_rq从全局时间池申请的时间片剩余时间，当剩余时间小于等于0的时候，就需要重新申请时间片。
> 1. 当cfs_rq被throttle的时候，方便统计被throttle的时间，需要记录throttle开始的时间。
...
如果使能CFS bandwidth control功能，cfs_bandwidth_used()返回1，`cfs_rq->runtime_enabled`值为1。\_\_account_cfs_rq_runtime()函数如下：
...
> 1. 如果cfs_rq剩余运行时间还有，那么没必要向全局时间池申请时间片。
> 1. 如果cfs_rq可运行时间不足，assign_cfs_rq_runtime()负责从全局时间池中申请时间片。
> 1. 如果全局时间片时间不够，就需要throttle当前cfs_rq。当然这里是设置TIF_NEED_RESCHED flag。在后面throttle。
...
> 1. 如果该cfs_rq不限制带宽，那么quota的值为RUNTIME_INF，既然不限制带宽，自然时间池的时间是取之不尽用之不竭的，所以申请时间片一定成功。
> 1. 确保定时器是打开的，如果关闭状态，就打开定时器。该定时器会在定时时间到达后，重置全局时间池可用剩余时间。
> 1. 申请时间片成功，全局时间池剩余可用时间更新。
> 1. cfs_rq剩余可用时间增加。
...
> 1. 如果该cfs_rq已经被throttle，这不需要重复操作。
> 1. throttle_cfs_rq()函数是真正throttle的操作，throttle核心函数。
...
> 1. `task_group`可以父子关系嵌套。walk_tg_tree_from()函数功能是顺着cfs_rq->tg往下便利每一个child task_group，并且对每个task_group调用tg_throttle_down()函数。tg_throttle_down()负责增加`cfs_rq->throttle_count`计数。
> 1. 从依附的cfs_rq的红黑树上删除。
> 1. 如果qcfs_rq运行的进程只有即将被dequeue的se一个的话，那么parent se也需要dequeue。如果`qcfs_rq->load.weight`不为0，说明qcfs_rq就绪队列上运行的进程不止se一个，那么parent se理所应当不能被dequeue。
> 1. 设置throttle标志位。
> 1. 记录throttle时刻。
> 1. 被throttle的cfs_rq加入cfs_b链表中，方便后续unthrottle操作可以找到这些已经被throttle的cfs_rq。
...
> 1. throttled标志位清零。
> 1. throttled_time记录cfs_rq被throttle的总时间，throttled_clock在throttle_cfs_rq()函数中记录开始throttle时刻。
> 1. 从链表上删除自己。
> 1. tg_unthrottle_up()函数是tg_throttle_down()函数的反操作，递减`cfs_rq->throttle_count`计数。
> 1. 如果unthrottle的cfs_rq上没有进程，那么无需进行enqueue操作。`cfs_rq->load.weight`为0就代表就绪队列上没有可运行的进程。
> 1. 将调度实体入队，这里的for循环操作和throttle_cfs_rq()函数的dequeue操作对应。
...
>
> 1. 归返全局时间池slack_runtime时间。
>
> 1. 开启slack_timer定时器条件有2个（从注释可以得知，使用定时器的原因是当前持有rq->lock锁）。
>
...
1. parent task_group在一个周期时间内消耗完自己的quota
...
1. 设置task_group带宽100%
   ...
   1. echo 250000 > cpu.cfs_period_us /\* period = 250ms \*/

1. 设置task_group带宽200%（多核系统）
...
1. 设置task_group带宽20%
   ...
   1. `echo 50000 > cpu.cfs_period_us /* period = 50ms */`
