...
本文主要以\_\_alloc_workqueue_key函数为主线，描述CMWQ中的创建一个workqueue实例的代码过程。

# 二、WQ_POWER_EFFICIENT的处理

\_\_alloc_workqueue_key函数的一开始有如下的代码：
...
>
> struct pool_workqueue \_\_percpu \*cpu_pwqs;  －－－－－指向per cpu的pool workqueue\
> struct pool_workqueue \_\_rcu \*numa_pwq_tbl\[\]; －－－－指向per node的pool workqueue\
> };
...
> }\
> return 0;\
> } else if (wq->flags & \_\_WQ_ORDERED) {－－－－－ordered unbound workqueue的处理\
...
> return -EINVAL;
>
> if (WARN_ON((wq->flags & \_\_WQ_ORDERED) && !list_empty(&wq->pwqs)))\
> return -EINVAL;
...
1. 在有交集的情况下，如果A==B，说明work只会在该node下运行，并且该node的任意一CPU均可处理，这样理解对吗。如果对的话，我觉得也应该创建per node，因为别的node不执行该work;如果不对，还望大神指点一二
...
## @linuxer：-------------------<br /> 在这种情况下，该unbound wq实际上是期望在node 1的任何一个cpu上运行而不在其他cpu上运行，这时候，创建per node pwq是没有意义的，使用default就OK了。
