...
1. 基于该library，实现devmd工具，可以显示指定的某个或者某些物理地址的内容。

1. 基于该library，实现devmcpy工具，可以将指定地址的内容，copy到另一个地址。类似memcpy。

1. 基于该library，实现devmset工具，可以将指定地址设置为某一个value。类似memset。

1. 基于该library，实现devmload和devmsave工具，将指定文件的内容加载到指定物理地址，以及从指定物理地址dump数据到指定的文件。
...
\_\_pgprot_modify(prot, PTE_ATTRINDX_MASK, PTE_ATTRINDX(MT_DEVICE_nGnRnE) | PTE_PXN | PTE_UXN)\
