...
有一点我们可能比较迷惑，在第一次执行backtrace的时候，main函数所在的栈帧编号为0，而第二次执行的时候，main函数的栈帧为1，而print函数的栈帧为0，这是因为_与栈的向下增长_规律一致，我们只需要记住_编号最小帧号就是最近一次调用的函数\_。
...
1. b test.cc:32 在第32行加上断点(效果与b 32一致)

1. info b 输出所有的断点信息

1. r 程序开始运行，并在第一个断点处暂停

1. c 执行c命令，在第二个断点处暂停，在第一个断点和第二个断点之间，创建了两个线程t1和t2

1. info threads 输出所有的线程信息，从输出上可以看出，总共有3个线程，分别为main线程、t1和t2
...
1. set follow-fork-mode child:指定调试子进程模式

1. r:运行程序，直接运行程序，此时会进入子进程，然后执行while循环

1. ctrl + c:通过该命令，可以使得GDB收到SIGINT命令，从而暂停执行while循环

1. n(next):继续执行，进而进入到while循环的条件判断处

1. show follow-fork-mode:再次执行该命令，通过输出可以看出，当前处于child模式下
...
- enum \_\_ptrace_request request：决定了系统调用的功能，几个主要的选项：
