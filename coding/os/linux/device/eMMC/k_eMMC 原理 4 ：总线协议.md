...
1. Host 将采样时间点重置为默认值
1. Host 向 eMMC Device 发送 Send Tuning Block 命令
1. eMMC Device 向 Host 发送固定的 Tuning Block 数据
1. Host 接收到 Tuning Block 并进行校验
1. Host 修改采样时点，重新从第 2 步开始执行，直到 Host 获取到一个有效采样时间点区间
1. Host 取有效采样时间点区间的中间值作为采样时间点，并推出 Tuning 流程
...
1. [SD/MMC Controller, Hard Processor System (HPS) Technical Reference Manual (TRM)](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.296.5338&rep=rep1&type=pdf) \[PDF\]
1. [CRC wiki](https://en.wikipedia.org/wiki/Cyclic_redundancy_check) \[WEB\]
