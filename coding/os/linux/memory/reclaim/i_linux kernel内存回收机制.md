...
1. 我这边碰到一个问题，就是有时候会有突发性的GFP_ATOMIC标志的大量分配内存的需求，虽然每次分配只要一个页面，但会一下子要求分配很多。这种情况下我经常看到即使kswapd在工作，似乎回收速度也赶不上分配的速度，因此会造成分配失败。这个有什么办法能改善么？谢谢！
...
## @ericzhou：你好，请教一个问题。<br /> 当回收一个页框时，怎么知道这个页框是否在被使用呢？
