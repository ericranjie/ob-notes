...
（3）没有找到reserved-memory节点之前，of_scan_flat_dt会不断的遍历下一个节点，而在\_\_fdt_scan_reserved_mem函数中返回0表示让搜索继续，如果返回1，表示搜索停止。
...
int \_\_init early_init_dt_scan_memory(unsigned long node, const char \*uname, int depth, void \*data)\
{\
while ((endp - reg) >= (dt_root_addr_cells + dt_root_size_cells)) {\
u64 base, size;\
base = dt_mem_next_cell(dt_root_addr_cells, ®);\
...
1. 如果你使用u-boot来引导kernel, 那么在booti的过程中，u-boot会将环境变量bootargs中的mem=xxxM解析出来，并且写入dtb中。 故kernel初始化时还是能scan到这个memory node.
