...
1. p2拷贝自p1，与p1共享同一个引用计数，将之增加为2

1. p4并非p1的拷贝，因此在构造的同时又为资源对象创建了另外一个外部引用计数，并将之置为1

1. 在作用域结束时，p4析构，由其维护的额外的引用计数降为0，导致资源对象被析构

1. 然后p2析构，对应的引用计数降为1

1. 接着p1析构，对应的引用计数也归零，于是p1在临死之前再次释放资源对象 最后，由于资源对象被二次释放，程序崩溃
