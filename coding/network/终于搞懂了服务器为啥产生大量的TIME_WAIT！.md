...
![图片](https://mmbiz.qpic.cn/mmbiz_gif/2rMyvdWluHvTBYyBWZhW4pP5FT0l7LebJwy5efjtuOgicSqeG8a7Ym9pPaXnfmXI2IMzM0QTPDUMtKviaOSshCSw/640?wx_fmt=gif&wxfrom=13&tp=wxpic)

写在开头，大概 4 年前，听到运维同学提到 TIME_WAIT 状态的 TCP 连接过多的问题，但是当时没有去细琢磨；最近又听人说起，是一个新手进行压测过程中，遇到的问题，因此，花点时间，细深究一下。 

![图片](https://mmbiz.qpic.cn/mmbiz_png/QFzRdz9libEYvO4icXe2ZG6mhMq8STawnibhpN2NxiceZkYicOlCA8SnYYWCQiasByWo6fquwqXXClQuGZUkVibYHKD8g/640?wx_fmt=png&wxfrom=13&tp=wxpic)
...
`Mac 下，查询 TCP 连接状态的具体命令：   // Mac 下，查询 TCP 连接状态   $ netstat -nat |grep TIME_WAIT      // Mac 下，查询 TCP 连接状态，其中 -E 表示 grep 或的匹配逻辑   $ netstat -nat | grep -E "TIME_WAIT|Local Address"   Proto Recv-Q Send-Q Local Address Foreign Address (state)   tcp4 0 0 127.0.0.1.1080 127.0.0.1.59061 TIME_WAIT      // 统计：各种连接的数量   $ netstat -n | awk '/^tcp/ {++S[$NF]} END {for(a in S) print a, S[a]}'   ESTABLISHED 1154   TIME_WAIT 1645   `
